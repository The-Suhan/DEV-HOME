    ...
    <div class="container py-5 d-flex justify-content-center">
        <div class="card bg-dark border-secondary shadow-lg" style="width: 500px; border-radius: 15px;">
            <div class="card-header border-secondary bg-transparent text-center py-3">
                <h4 class="text-info fw-bold mb-0">Create New Post</h4>
            </div>
            <div class="card-body p-4 text-white">
                <form action="{{ route('posts.store') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div class="mb-3">
                        <label class="text-white-50 small d-block mb-2">Post Type:</label>
                        <div class="btn-group w-100" role="group">
                            <input type="radio" class="btn-check" name="type" id="typeImg" value="image" checked>
                            <label class="btn btn-outline-info" for="typeImg">📸 Photo</label>

                            <input type="radio" class="btn-check" name="type" id="typeVid" value="video">
                            <label class="btn btn-outline-info" for="typeVid">🎥 Video</label>
                        </div>
                    </div>

                    <div class="mb-3">
                        <label class="text-white-50 small">Select File:</label>
                    ...
                    <button type="submit" class="btn btn-info w-100 fw-bold">Share Post</button>
                </form>
            </div>
        </div>
    </div>
